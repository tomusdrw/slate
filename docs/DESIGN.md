...
A provider is the adapter between one upstream system and the runtime. Version 1 has four provider ids: `direct`, which is always present; `ha`, which is present but `unconfigured` until it has credentials; `shelly` (§5.9), which is present and `unconfigured` until a dashboard binds a device to it; and `tuya`, which is present but `unconfigured` until its Tuya cloud credentials are entered. The common core knows only five operations:
