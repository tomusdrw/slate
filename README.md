...
**Shelly**, which the panel polls on the LAN by itself; and **Tuya**, which
drives Smart Life devices through the Tuya cloud. None is required by the
others. With Shelly or Tuya the panel needs nothing else running at all, and
apart from the daily update check, the only traffic that leaves the local
network is what a configured Tuya provider sends to the Tuya cloud.
